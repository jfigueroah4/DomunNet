...
# We know the array ends with "];\n" or "]\n" before export default.
# Let's just find "export default function MantenimientoTablas() {"
split_token = "export default function MantenimientoTablas() {"
header = content.split(split_token)[0]
        ...
        {data.length > 0 && (
          <div className="p-3 border-t border-gray-100 bg-white flex flex-col sm:flex-row items-center justify-between gap-4 flex-shrink-0 z-20">
            ...
            <div className="flex items-center gap-1">
              <button 
                onClick={() => setCurrentPage(prev => max(prev - 1, 1))}
              ...
              <button 
                onClick={() => setCurrentPage(prev => min(prev + 1, totalPages))}
...
final_header = header
if "import { EstadoVacio } from" not in final_header:
    final_header = final_header.replace("import { useCustomToast } from '@/hooks/useCustomToast'", "import { useCustomToast } from '@/hooks/useCustomToast'\nimport { EstadoVacio } from '@/components/ui/EstadoVacio'")

with open(path, 'w', encoding='utf-8') as f:
    f.write(final_header + new_component)
