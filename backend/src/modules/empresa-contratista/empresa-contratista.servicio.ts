...
function datosUsuario(c: NonNullable<EmpresaContratistaPayload['contacto']>) { return { primer_nombre: c.primer_nombre, segundo_nombre: c.segundo_nombre || '', primer_apellido: c.primer_apellido, segundo_apellido: c.segundo_apellido || '', correo: c.correo, telefono: c.telefono, rol: 'Contratante', estado: 'Activo' as const, username: c.username, contrasena: c.password || undefined, fecha_nacimiento: c.fecha_nacimiento, direccion: c.direccion } }
export async function crearEmpresaContratista(payload: EmpresaContratistaPayload) {
  ...
  try {
    ...
    if (usuarioId && payload.contacto) {
      const contacto = await clienteSupabase.from('contacto_contratista').insert({ empresa_contratista_id: data.id, usuario_id: usuarioId, cargo: payload.contacto.cargo || 'Representante' }).select('id').single()
...
export async function actualizarEmpresaContratista(id: string, payload: EmpresaContratistaPayload) {
  ...
  if (c?.usuario_id && payload.contacto) {
