...
# I will replace the exact block for SelectorMapaInteractivo
# Find the start of the function and the end by looking for // COMPONENTE: Configuración inicial del Plan de Trabajo (or similar, // COMPONENTE: Configuraci)

start_marker = '// Selector de mapa interactivo estilo Google Maps'
end_marker = '// COMPONENTE: Configuración inicial del Plan de Trabajo'
end_marker_fallback = '// COMPONENTE: Configuraci'
...
if start_idx != -1 and end_idx != -1:
    new_mapa = '''// Selector de mapa interactivo estilo Google Maps
function SelectorMapaInteractivo({
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(async () => {
      setIsGeocoding(true)
      try {
        const query = ${direccion}, Guatemala
  ...
  return (
    <div className="space-y-2 rounded-lg border border-gray-200 bg-gray-50/50 p-2.5">
      <div className="relative">
        <label className="mb-1 block text-[10px] font-bold uppercase text-gray-700">Dirección (Texto Corto) <span className="text-[#9B0F06]">*</span></label>
        <div className="relative">
          <input
            type="text"
            value={direccion}
          ...
          />
          {isGeocoding && <Loader2 size={12} className="absolute right-3 top-2.5 animate-spin text-gray-400" />}
        </div>
      ...
      <div>
        <div className="mb-1 flex flex-wrap items-center justify-between gap-1.5">
          <label className="mb-1 block text-[10px] font-bold uppercase text-gray-700">Selector de Mapa Interactivo (Punto o Tramo Exacto)</label>
  ...
  '''
    content = content[:start_idx] + new_mapa + content[end_idx:]
