new_mapa = r"""function SelectorMapaInteractivo({
  direccion,
  setDireccion,
  errors,
  setErrors,
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(async () => {
      ...
      try {
        ...
        const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query)}&format=json&countrycodes=gt&limit=5`, {
      ...
      <div className="relative">
        ...
        <div className="relative">
          <input
            ...
            className={`w-full rounded-xl border ${errors.direccion ? 'border-red-500' : 'border-gray-200'} bg-white px-3 py-2 text-xs text-gray-700 focus:border-[#9B0F06] focus:outline-none focus:ring-1 focus:ring-[#9B0F06]`}
        ...
        <div className="mb-1 flex flex-wrap items-center justify-between gap-1.5">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className={`rounded px-1.5 py-0.5 text-[8px] font-semibold border flex items-center gap-1 ${
                modoSeleccion === 'tramo'
