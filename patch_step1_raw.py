...
ubicacionHtml = r'''<div className="space-y-2">
...
# Fallback with unicode if above fails
content = content.replace(
  '<div className="space-y-2">\n                  <div>\n                    <label className={labelClass}>Ubicaci\u00f3n F\u00edsica',
  ubicacionHtml.replace('Ubicación Física', 'Ubicaci\u00f3n F\u00edsica')
)

# 3. Update Validation logic in handleAvanzarPaso
valHtml = r'''if (!ubicacionFisica.trim()) { newErrors.ubicacionFisica = true; isValid = false }
...
sig_old = r'''<SelectorMapaInteractivo
                    direccion={direccion}
                  ...
                  />'''
sig_new = r'''<SelectorMapaInteractivo
                    direccion={direccion}
                    setDireccion={setDireccion}
                    errors={errors}
                    setErrors={setErrors}
