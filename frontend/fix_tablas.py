...
# 1. Fix Mojibake in TABLAS_MANTENIMIENTO
old_tablas = """const TABLAS_MANTENIMIENTO = [
  // 1. Catǟlogos Operacionales
  { id: 'catalogo', nombre: 'Catǟlogos', endpoint: '/mantenimiento/catalogo', grupo: 'Operacionales' },
  { id: 'catalogo_item', nombre: 'Ítems de Catǟlogo', endpoint: '/mantenimiento/catalogo_item', grupo: 'Operacionales' },
  
  // 2. Proyectos
  { id: 'proyecto', nombre: 'Proyectos (Config)', endpoint: '/mantenimiento/proyecto', grupo: 'Proyectos' },
  { id: 'fase_proyecto', nombre: 'Fases de Proyecto', endpoint: '/mantenimiento/fase_proyecto', grupo: 'Proyectos' },
  { id: 'categoria_actividad', nombre: 'Categorǟas de Actividad', endpoint: '/mantenimiento/categoria_actividad', grupo: 'Proyectos' },
  { id: 'renglon_trabajo', nombre: 'Renglones de Trabajo', endpoint: '/mantenimiento/renglon_trabajo', grupo: 'Proyectos' },
  { id: 'modificativo_renglon', nombre: 'Modificativos Renglǟn', endpoint: '/mantenimiento/modificativo_renglon', grupo: 'Proyectos' },
  { id: 'catalogo_descuento_tecnico', nombre: 'Descuentos Tǟcnicos', endpoint: '/mantenimiento/catalogo_descuento_tecnico', grupo: 'Proyectos' },

  // 3. Estructura Geogrǟfica
  { id: 'departamento', nombre: 'Departamentos', endpoint: '/mantenimiento/departamento', grupo: 'Estructura Geogrǟfica' },
  { id: 'municipio', nombre: 'Municipios', endpoint: '/mantenimiento/municipio', grupo: 'Estructura Geogrǟfica' },
  ...
  { id: 'especificacion_tecnica', nombre: 'Especificaciones Tǟcnicas', endpoint: '/mantenimiento/especificacion_tecnica', grupo: 'Laboratorio' },

  // 6. Configuraciǟn
  { id: 'unidad_medida', nombre: 'Unidades de Medida', endpoint: '/mantenimiento/unidad_medida', grupo: 'Configuraciǟn' },
  { id: 'parametro_proyecto', nombre: 'Parǟmetros Proyecto', endpoint: '/mantenimiento/parametro_proyecto', grupo: 'Configuraciǟn' },
  { id: 'suspension_plazo', label: 'Suspensiones de Plazo', endpoint: '/mantenimiento/suspension_plazo', grupo: 'Configuraciǟn' },
  { id: 'control_plazo', nombre: 'Controles de Plazo', endpoint: '/mantenimiento/control_plazo', grupo: 'Configuraciǟn' },
  { id: 'control_anticipo', nombre: 'Controles de Anticipo', endpoint: '/mantenimiento/control_anticipo', grupo: 'Configuraciǟn' },
  { id: 'cronograma_planificado', nombre: 'Cronogramas Planificados', endpoint: '/mantenimiento/cronograma_planificado', grupo: 'Configuraciǟn' },

  // 7. Seguridad/Auditorǟa
  { id: 'rol', nombre: 'Roles', endpoint: '/mantenimiento/rol', grupo: 'Seguridad/Auditorǟa' },
  { id: 'estado_usuario', nombre: 'Estados Usuario', endpoint: '/mantenimiento/estado_usuario', grupo: 'Seguridad/Auditorǟa' },
]"""
...
# Sometimes python find/replace fails on mojibake, so we'll regex or replace chunks
import re
content = re.sub(r"const TABLAS_MANTENIMIENTO = \[[^\]]*\]", new_tablas, content, flags=re.MULTILINE|re.DOTALL)

# 2. Fix the "Administra los catálogos" mojibake
...
# 3. Add dependenciasCount to columns exclusion
...
# 5. Replace actions logic for dependenciasCount
...
new_actions = """                      <td className="px-4 py-3 whitespace-nowrap text-right sticky right-0 bg-white z-10 border-l border-gray-100 shadow-[-4px_0_10px_rgba(0,0,0,0.03)] group-hover:bg-gray-50 transition-colors">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => handleView(row)} className="p-1 text-gray-400 transition-colors hover:text-[#9B0F06]" title="Ver detalle">
                            <Eye size={12} />
                          </button>
                          <button onClick={() => handleEdit(row)} className="p-1 text-gray-400 transition-colors hover:text-green-600" title="Editar">
                            <Edit2 size={12} />
                          </button>
                          {row.dependenciasCount > 0 ? (
                            <button disabled className="p-1 text-gray-300 opacity-50 cursor-not-allowed" title={No se puede eliminar: tiene  dependencia(s) asignada(s).}>
                              <Trash2 size={12} />
                            </button>
                          ) : (
                            <button onClick={() => handleDelete(row)} className="p-1 text-gray-400 transition-colors hover:text-red-600" title="Eliminar">
                              <Trash2 size={12} />
                            </button>
                          )}
                        </div>
...
# 6. Change table styling to look like UsuarioTabla
...
# Fix API calls string interpolation (since it got messed up before)
content = re.sub(r"api\.get\( \+ '' \+ \$\{selectedTable\.endpoint\}\?\$\{params\.toString\(\)\} \+ '' \+\)", r"api.get(${selectedTable.endpoint}?)", content)
content = re.sub(r"api\.delete\( \+ '' \+ \$\{selectedTable\.endpoint\}/\$\{recordToDelete\.id\} \+ '' \+\)", r"api.delete(${selectedTable.endpoint}/)", content)

# I'll just write a direct string replace if there are no backticks
content = content.replace("await api.get( + '' + ${selectedTable.endpoint}? + '' + )", "await api.get(${selectedTable.endpoint}?)")
content = content.replace("await api.delete( + '' + ${selectedTable.endpoint}/ + '' + )", "await api.delete(${selectedTable.endpoint}/)")

# Add Filters Dropdown next to search
