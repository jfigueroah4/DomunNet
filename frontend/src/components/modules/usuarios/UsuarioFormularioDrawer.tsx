...
export function UsuarioFormularioDrawer({
  ...
  const handleGuardar = () => {
    ...
    const regexLetras = /^[A-Za-zÃƒÂ-Úá-úñÑ\s]+$/
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                onChange={handleChange}
                placeholder="********"
                className="w-full h-9 border border-gray-200 rounded-lg pl-2.5 pr-10 text-xs text-gray-700 bg-white focus:outline-none focus:border-[#9B0F06] transition-colors"
              />
              <button
                type="button"
                onClick={() => setShowPassword(!showPassword)}
                className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 focus:outline-none h-6 w-6 flex items-center justify-center"
              >
