...
# Read the parsed filters from the generated text file or recreate them.
import ast
with open(r"C:\DomunNet\filters_report.txt", "r", encoding="utf-8") as f:
    report = f.read()

# We can parse the exact rules again or just pull from schema quickly
...
# Replace block
content = re.sub(r'const TABLAS_MANTENIMIENTO:.*?\s=\s*\[.*?\](?=\s*export default function)', out_ts + '\n\n', content, flags=re.DOTALL)

with open(fe_path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Frontend repaired.")
